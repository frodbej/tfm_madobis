...
# We are using robust standarization that uses median and IQR that are robust
# to outliers
...
ggplot(gene_group_df, aes(x=Image_Metadata_Gene, y=Cell_AreaShape_Area)) + 
  geom_point() + ylim(c(-1,2.25)) +
  geom_text(aes(label=ifelse(Cell_AreaShape_Area > 1.25 | Cell_AreaShape_Area < -1.25,
...
ggplot(gene_group_df, aes(x=Image_Metadata_Gene, y=FilteredNuclei_AreaShape_Area)) + 
  geom_point() +
  geom_text(aes(label=ifelse(FilteredNuclei_AreaShape_Area > 1.5 | FilteredNuclei_AreaShape_Area < -1.5,
                             as.character(Image_Metadata_Gene),'')),hjust=-0.2,vjust=0.2, size=4.5) +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
ggplot(gene_group_df, aes(x=Image_Metadata_Gene, y=FilteredNuclei_AreaShape_Perimeter)) + 
  geom_point() +
  geom_text(aes(label=ifelse(FilteredNuclei_AreaShape_Perimeter > 1.25 | FilteredNuclei_AreaShape_Perimeter < -1.25,
                             as.character(Image_Metadata_Gene),'')),hjust=-0.2,vjust=0.2,size=4.5) +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
ggplot(gene_group_df, aes(x=Image_Metadata_Gene, y=Cell_AreaShape_FormFactor)) + 
  geom_point() +
  geom_text(aes(label=ifelse(Cell_AreaShape_FormFactor > 1 | Cell_AreaShape_FormFactor < -1,
                             as.character(Image_Metadata_Gene),'')),hjust=-0.2,vjust=0.2, size=4.5)+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
ggplot(gene_group_df, aes(x=Image_Metadata_Gene, y=Cell_AreaShape_Compactness)) + 
  geom_point() +
  geom_text(aes(label=ifelse(Cell_AreaShape_Compactness > 1 | Cell_AreaShape_Compactness < -1,
                             as.character(Image_Metadata_Gene),'')),hjust=-0.1,vjust=0.2, size=4.5)+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) + labs(y="Cell Compactness Length Z-Scores")
